...
// Lifted Magic Constants
const STORAGE_KEY = "sheger_transit_data";
const PHONE_REGEX = /^(?:\+251|0)9\d{8}$/;

// Validation Function (Returns error string or empty string on success)
function validate({ name, phone }) {
  if (!name.trim()) return "Please enter your name.";
  if (!PHONE_REGEX.test(phone.trim())) return "Enter a valid Ethiopian phone number (e.g., 0911223344 or +251911223344).";
  if (state.favorites.length === 0) return "Your saved list is empty.";
  return "";
}

// Submitting Order & Clearing State
function placeOrder(formData) {
  ...
  if (errorMsg) {
    ...
    return; // Early guard clause return
  }
  ...
  // State mutation, local persistence, and notification
  state.favorites = [];
  save();
  render();
  alert(`Pass booked successfully! Total: ${order.total} ETB`);
}
function renderFavorites() {
  const favEl = document.querySelector("#favorites-container");
  if (!favEl) return;

  // Guard clause for empty state
  if (state.favorites.length === 0) {
    favEl.innerHTML = "<p class='empty'>Your saved favorites list is empty.</p>";
    return;
  }

  // Normal render logic
  favEl.innerHTML = state.favorites
    .map(item => `<div class="fav-item">${item?.routeNumber ?? "Route"} - ${item?.fare ?? 0} ETB</div>`)
    .join("");
}
